...
class ChatDetailView(DetailView):
    ...
    def post(self, request, *args, **kwargs):
        chat = self.get_object()
        message_form = MessageForm1234(request.POST)
...
class CreateDialogView(View):
    ...
    def post(self, request, chat_id, username):
        form = MessageForm1234(data=request.POST)
        ...
        if dialog.exists():
            ...
            return redirect(
                reverse('messages_main:messages', kwargs={'chat_id': dialogs.id, 'username': recipient.username}))
        else:
            # Создаем новый чат, если он не существует
            dialog = Chat.objects.create(type=Chat.DIALOG)
    ...
    def send_message(self, request, chat_id, username):
        ...
        if self.is_blocked(sender, recipient):
            ...
            if form.is_valid():
                ...
                message.photo = photo
                message.save()
                return redirect(reverse('messages_main:messages', kwargs={'chat_id': chat_id, 'message_id': message.id, 'username': username}))
...
def chat_list(request):
   Dialog_chats = Chat.objects.filter(members=request.user, type='D')
   Group_chats = Chat.objects.filter(members=request.user, type='C')
...
@app.route('/messages/send-message/', methods=['POST'])
